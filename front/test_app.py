...
check(
    "FakeTaskRepository dziedziczy po TaskRepository (wzorzec Repository)",
...
check("get_all() filtruje is_deleted=True domyslnie", all(not t.is_deleted for t in repo.get_all()))
check("get_all(include_deleted=True) pokazuje usuniete", any(t.is_deleted for t in repo.get_all(include_deleted=True)))
check(
    "get_habits zwraca tylko zadania z recurrence_rule", all(t.recurrence_rule is not None for t in repo.get_habits())
)
check("get_habits pomija is_deleted", all(not t.is_deleted for t in repo.get_habits()))
...
check("Pozycje nawigacyjne: Dziś, Wszystkie, Nawyki", items_text == ["Dziś", "Wszystkie", "Nawyki"])
...
check("Pierwsza pozycja kolorowa to 'Priorytetowe' (hardcoded)", color_texts[0] == "Priorytetowe")
check(
    "Priorytetowe ma kolor PRIORYTETOWE_COLOR (czerwony stonowany)",
    color_rows[0]["widget"].cget("bg") == PRIORYTETOWE_COLOR,
)
check("Sa 3 kategorie z fake_data po Priorytetowe", color_texts[1:] == ["Kategoria 1", "Kategoria 2", "Kategoria 3"])

# kazda kolorowa pozycja ma ContextMenuButton z opcjami Edytuj/Dodaj/Usun
first_cat = color_rows[1]["widget"]
check("Kazda kategoria ma menu_btn (ContextMenuButton)", isinstance(first_cat.menu_btn, ContextMenuButton))
...
for e in test_sidebar.entries:
    if e["kind"] == "item":
        e["widget"].click()
check(
    "Klik na pozycje nawigacyjne woluje on_select z odpowiednim kluczem",
...
expected_color_keys = [
    KEY_PRIORYTETOWE,
    f"{KEY_CATEGORY_PREFIX}1",
    f"{KEY_CATEGORY_PREFIX}2",
    f"{KEY_CATEGORY_PREFIX}3",
]
check(
    "Klik na kolorowa pozycje woluje on_select z key 'priorytetowe' lub 'kategoria_N'",
...
for label, callback in first_color_row["widget"].menu_btn._opts:
    callback()
check(
    "Wszystkie 3 opcje menu (Edytuj/Dodaj/Usun) na kategorii wolaly on_category_menu", len(clicks["category_menu"]) == 3
)
...
check("Tytul widoku = 'Nawyki'", app.view.header.cget("text") == "Nawyki")
check("Sort options = Data/Priorytet/Kategorie", [opt[0] for opt in app.view.toolbar.lbl_sortuj._opts] == SORT_OPTIONS)
check(
    "Filter options = Priorytet/Kategoria/Status/Data",
    [opt[0] for opt in app.view.toolbar.lbl_filtruj._opts] == FILTER_OPTIONS,
)
check("BRAK 'Widok' w toolbar (tylko Dzis ma)", app.view.toolbar.lbl_widok is None)
check("MA przycisk '+' w toolbar", app.view.toolbar.lbl_plus is not None)


print("\n=== 6. NawykiView - tylko zadania cykliczne, bez usunietych ===")
titles = [r.task.title for r in app.view.task_rows]
check("Wszystkie wiersze maja recurrence_rule", all(r.task.recurrence_rule for r in app.view.task_rows))
check("'Stare zadanie' (is_deleted=True) nie wystepuje", "Stare zadanie" not in titles)
# pomocniczo: ile sie wyswietla
print(f"  zadania w NawykiView: {titles}")
check(
    "Mamy 4 nawyki (Poranny trening, Sprzatanie, Mycie samochodu, Codzienna medytacja)",
...
for row in app.view.task_rows:
    if row.task.priority:
        check(
            f"'{row.task.title}' (priority=True) ma kolor PRIORYTETOWE_COLOR czerwony",
...
for label, callback in test_view.toolbar.lbl_sortuj._opts:
    callback()
check("Sortuj opcje wywoluja on_sort z parametrami w kolejnosci", sort_clicks == SORT_OPTIONS)
for label, callback in test_view.toolbar.lbl_filtruj._opts:
    callback()
check("Filtruj opcje wywoluja on_filter z parametrami w kolejnosci", filter_clicks == FILTER_OPTIONS)
...
for label, callback in row.menu_btn._opts:
    callback()
check(
    "Wszystkie 3 opcje menu (Edytuj/Dodaj/Usun) wolaly on_task_menu(task)",
...
for w in (700, 1200, 1920):
    ...
    check(f"w={w}: font tytulu zadania w zakresie [16, 30] -> {task_font_size}", 16 <= task_font_size <= 30)
    check(f"w={w}: font sidebara w zakresie [12, 36] -> {sidebar_font_size}", 12 <= sidebar_font_size <= 36)


print("\n=== 12. minsize blokuje zmniejszanie ponizej 700px ===")
app.window.geometry("300x300")
app.window.update_idletasks()
check("Okno nie schodzi ponizej minsize (700x400)", app.window.winfo_width() >= 700)
